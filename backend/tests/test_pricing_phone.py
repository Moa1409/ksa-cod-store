...
def test_bundle_tiers():
    ...
    assert bundle_total(1) == 149  # cheapest unit (hair-perfume-mist)
...
def test_cart_subtotal_mixed():
    lines = [
        CartLine(slug="keratin-collagen-mask", qty=1),
        CartLine(slug="hair-perfume-mist", qty=1),
    ]
    assert cart_subtotal(lines) == BUNDLE_2


def test_cart_subtotal_single_product():
    lines = [CartLine(slug="keratin-collagen-mask", qty=1)]
    assert cart_subtotal(lines) == 219


def test_allocate_unit_prices():
    lines = [CartLine(slug="keratin-collagen-mask", qty=2)]
...
def test_product_prices():
    assert product_price("keratin-collagen-mask") == 219
    assert product_price("hair-perfume-mist") == 149
    assert product_price("hair-skin-nails-gummies") == 199
