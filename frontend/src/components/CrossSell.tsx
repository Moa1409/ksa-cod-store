        ...
        <div className="space-y-2">
          {items.map((p) => (
            <div key={p.slug} className="flex items-center gap-3 rounded-2xl border border-brand-rose/50 bg-white p-2">
              <Media label={p.gallery[0]} emoji={p.emoji} src={p.image} aspect="square" className="h-14 w-14 shrink-0 rounded-xl" />
      ...
      {items.map((p) => (
        <div key={p.slug} className="card flex items-center gap-4 p-3">
          <Link href={`/product/${p.slug}`}>
            <Media label={p.gallery[0]} emoji={p.emoji} src={p.image} aspect="square" className="h-20 w-20 rounded-xl" />
          </Link>
          <div className="min-w-0 flex-1">
